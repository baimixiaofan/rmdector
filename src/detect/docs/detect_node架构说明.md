...
**处理**：用 YOLOv8 模型（ONNX 格式）检测画面里的装甲板
**输出**：两个话题——

| 话题 | 类型 | 内容 |
|---|---|---|
| `/detect/image` | sensor_msgs/Image | 画了绿色检测框的图像（给人看的） |
...
processFrame()                  ★ 核心流水线，一帧图从头走到尾
  ...
  │  ├─ ④ drawBoxes()      在副本上画绿框和文字
  │  ├─ ⑤ 发布画框图        /detect/image
  │  └─ ⑥ publishDetectionResults()   → /detect/detections
```

**全文件只有 8 个函数**，其中真正要理解的是 `processFrame` 和它的三个子函数（①②③），其余都是模板/辅助。
...
| 输入 | 无 |
| 输出 | 无（把节点对象准备好） |
| 职责 | ① 读 5 个 ROS 参数 ② 加载 ONNX 模型 ③ 创建 2 个订阅者 + 2 个发布者 |
...
**大白话**：启动时把所有家当准备好——模型装进内存、订阅/发布的话题接好线。模型文件加载失败会直接抛异常，节点起不来。
...
2. **第二遍**：分别细看 letterbox / postprocess（这俩是核心算法，其余是模板）
